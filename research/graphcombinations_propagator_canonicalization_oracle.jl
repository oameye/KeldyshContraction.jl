...
function _gc_direct_graph(vs, graph_positions)
    ...
    return GC.DirectedGCGraph(edges, length(graph_positions)),
    KC.position_labels(graph_positions),
    simple
end
...
function canonicalize_with_permutation(
    vs::Vector{T}, graph_positions, permutation
) where {T}
...
@testset "GC witness consistency" begin
    for (label, vs) in cases
        ...
        @test all(
            witnesses.canonical_to_old[witnesses.old_to_canonical[old_vertex]] ==
            old_vertex for old_vertex in 1:n
        )

        nauty_perm = KC.canonicalization_permutation(vs, graph_positions)
        gc_original, gc_original_map = canonicalize_with_permutation(
            vs, graph_positions, witnesses.canonical_to_old
        )
        gc_inverse, gc_inverse_map = canonicalize_with_permutation(
            vs, graph_positions, witnesses.old_to_canonical
        )
        nauty_output, nauty_map = canonicalize_with_permutation(
            vs, graph_positions, nauty_perm
        )
...
for r in Iterators.take(mismatches, 12)
    ...
    println(
        "  path=",
        w.use_direct ? "direct" : "colored",
        " simple=",
        w.simple,
        " direct-aut=",
        w.direct_automorphisms,
        " final-aut=",
        w.final_automorphisms,
    )
    ...
    println(
        "  map equality: original=",
        r.original_map_equal,
        " inverse=",
        r.inverse_map_equal,
        "; output equality: original=",
        r.original_output_equal,
        " inverse=",
        r.inverse_output_equal,
    )
end
