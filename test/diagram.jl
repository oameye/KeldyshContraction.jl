...
@testset "construction" begin
    ...
    @inferred Diagrams{KC.ComplexRationals,Boson,3,0}()

    @qfields field::Boson
    c, q = field[Classical], field[Quantum]
    vs = Contraction{Boson}[(c(Out()), bar(q)), (c, bar(q)), (c, bar(q)(In()))]
...
@testset "static order entry point" begin
    using KeldyshContraction:
        In, Out, DressedPropagator, SelfEnergy, Diagram, Diagrams, FixedVector, matrix
    ...
    @test diagrams isa Diagrams{KC.ComplexRationals,Boson,3,0}
    vacuum_diagrams = @inferred KC._wick_contraction(L.lagrangian, Val(2))
    ...
    @test_throws MethodError SelfEnergy(propagator, Val(1))
...
@testset "prefactor multiplication" begin
    ...
    ds = Diagrams(Dict(d => KC.ComplexRationals(1.0)))
    ds2 = ds * 2.0
    ds2′ = 2.0 * ds
    @test isequal(ds2, ds2′)
    @test ds2 isa Diagrams{ComplexF64,Boson,3,0}
end
...
@testset "is_connected" begin
    ...
    vs = KC.Contraction{Boson}[(c(Out()), bar(q)), (c, bar(q)), (c, bar(q)(In()))]
    @test KC.is_connected(vs)

    vs2 = KC.Contraction{Boson}[(c, bar(q))]
    @test KC.is_connected(vs2)

    vs3 = KC.Contraction{Boson}[(c, bar(q)), (c(Out()), bar(q)(In()))]
    @test !KC.is_connected(vs3)
end
...
@testset "bulk multiplicity" begin
    ...
    vs = map(tt -> KC.index.(tt), vs)
    @test KC.bulk_multiplicity(vs) == Int[]
    ...
    vs2 = map(tt -> KC.index.(tt), vs2)
    @test KC.bulk_multiplicity(vs2) == Int[1]
    ...
    vs3 = map(tt -> KC.index.(tt), vs3)
    @test KC.bulk_multiplicity(vs3) == Int[2]
    ...
    vs4 = map(tt -> KC.index.(tt), vs4)
    @test KC.bulk_multiplicity(vs4) == Int[2, 1, 0]
end

@testset "vertices" begin
    edges1 = Tuple{Int,Int}[]
    @test KC.vertices(edges1) == Set{Int}()

    edges2 = [(1, 2)]
    @test KC.vertices(edges2) == Set([1, 2])

    edges3 = [(1, 2), (2, 3), (3, 4)]
    @test KC.vertices(edges3) == Set([1, 2, 3, 4])

    edges4 = [(1, 1), (2, 2)]
    @test KC.vertices(edges4) == Set([1, 2])

    edges5 = [(1, 2), (2, 3), (1, 3)]
    @test KC.vertices(edges5) == Set([1, 2, 3])
end

@testset "connected components" begin
    edges1 = Tuple{Int,Int}[]
    vertices1 = Set{Int}()
    @test KC.connected_components(vertices1, edges1) == Vector{Set{Int}}()

    vertices2 = Set([1])
    edges2 = Tuple{Int,Int}[]
    comps2 = KC.connected_components(vertices2, edges2)
    @test length(comps2) == 1
    @test comps2[1] == Set([1])

    edges3 = [(1, 2), (2, 3), (3, 4)]
    vertices3 = KC.vertices(edges3)
    comps3 = KC.connected_components(vertices3, edges3)
    @test length(comps3) == 1
    @test Set([1, 2, 3, 4]) ∈ comps3

    edges4 = [(1, 2), (3, 4), (5, 6)]
    vertices4 = KC.vertices(edges4)
    comps4 = KC.connected_components(vertices4, edges4)
    ...
    vertices5 = KC.vertices(edges5)
    comps5 = KC.connected_components(vertices5, edges5)
    ...
    comps6 = KC.connected_components(vertices6, edges6)
...
@testset "Diagrams unique collection and prefactor sum" begin
    ...
    c2 = (ϕᶜ, bar(ϕᶜ))
    c3 = (ϕᶜ(Out()), bar(ϕᴾ))
    contractions1 = Contraction{Boson}[c1, c2, c3]
    contractions2 = Contraction{Boson}[c1, c2, c3]
    contractions3 = Contraction{Boson}[c2, c3, c1]
    contractions4 = Contraction{Boson}[c1, c3]
    ...
    diagrams = Diagrams{Float64,Boson,3,0}()
