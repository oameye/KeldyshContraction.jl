...
"""
function DressedPropagator(L::InteractionLagrangian; order=1, simplify=true, kwargs...)
    ϕ = L.qfield
    ψ = L.cfield
    if order == 1 #-i*i
        keldysh = wick_contraction(ψ(Out()) * ψ'(In()), L, order; simplify=true, kwargs...)
        retarded = wick_contraction(ψ(Out()) * ϕ'(In()), L, order; simplify=true, kwargs...)
        advanced = wick_contraction(ϕ(Out()) * ψ'(In()), L, order; simplify=true, kwargs...)
