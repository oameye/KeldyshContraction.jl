...
`advanced_component` rather than depending on the storage fields. The same component
accessors remain valid after Fourier transformation, Wigner transformation, and kinetic
lowering (`FourierDressedPropagator`, `FourierSelfEnergy`, `WignerDressedPropagator`,
`WignerSelfEnergy`, and `KineticSelfEnergy`). The qualified `order`, `statistics`,
`parameters`, and `target_family` accessors expose provenance without widening the default
namespace.
