...
function pwave_branch_pairs(; shifted::Bool)
    ...
    ∂ψplus = shifted ? partial(ψ₁(minus), :x) + partial(ψ₂(minus), :x) : partial(ψ₁, :x) + partial(ψ₂, :x)
    ψplus_forward = shifted ? ψ₁(plus) + ψ₂(plus) : ψ₁ + ψ₂
    ∂ψplus_forward = shifted ? partial(ψ₁(plus), :x) + partial(ψ₂(plus), :x) : partial(ψ₁, :x) + partial(ψ₂, :x)
    ψminus = shifted ? ψ₁(plus) - ψ₂(plus) : ψ₁ - ψ₂
    ∂ψminus = shifted ? partial(ψ₁(plus), :x) - partial(ψ₂(plus), :x) : partial(ψ₁, :x) - partial(ψ₂, :x)
...
function fermionic_pwave_loss_lagrangian_second_order()
    Pplus_minus, Pplus_plus, Pminus_plus, Pplus_dagger, Pminus_dagger = pwave_branch_pairs(; shifted=true)

    # Same finite-Trotter Lindblad convention as the certified O(γp) oracle in #312.
    loss =
        (1 // 8) *
        im *
        ((Pplus_dagger - Pminus_dagger) * Pplus_minus - (Pplus_plus - Pminus_plus) * Pminus_dagger)
...
function generated_second_order_pipeline(G, parameter)
    ...
    return (; GF, ΣF, ΣW, kinetic, off_shell, spectral, canonical, reduced, occupation, quotient, kernel)
end
...
function research_second_order_census(result)
    ...
    return (; spectral_terms, canonical_regular, canonical_shifted, regular, blocked, trotter, occupation, quotient, kernel)
end
...
@testset "research: spinless-fermion p-wave second-order canonical compiler census" begin
    ...
    for parameter in parameters_expected
        ...
        @test all(
            ...
            (result.GF, result.ΣF, result.ΣW, result.kinetic, result.off_shell, result.spectral, result.canonical, result.reduced, result.occupation, result.quotient, result.kernel),
        )
