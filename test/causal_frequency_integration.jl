...
function causal_expression_denominator(
    coefficients::NTuple{N,Int}, energy, infinitesimal
) where {N}
...
@testset "expression-level causal frequency integration" begin
    ...
    @testset "zero prescription has deterministic precedence" begin
        expression = one_term_expression(
            1 // 1,
            [
                causal_expression_denominator(1, -Eₐ, -1),
    ...
    @testset "exact upper repeated pole integrates generically" begin
    ...
    @testset "double-pole residue differentiates remaining factors" begin
        upper = causal_expression_denominator(1, -Eₐ, -1)
        lower = causal_expression_denominator(1, -Eᵦ, 1)
        expression = one_term_expression(1 // 1, [upper, upper, lower])
        result = @inferred KC.integrate_causal_frequency_expression(expression, 1)
        @test KC.causal_frequency_integration_kind(result) === KC.CausalFrequencyIntegrated

        residual = KC._substitute_causal_frequency_pole(lower, upper, 1)
        expected = KC.CausalFrequencyExpression([
            KC.CausalFrequencyTerm(convert(KC.ComplexRationals, -im), [residual, residual])
        ])
        @test KC.causal_frequency_integration_expression(result) == expected
    end

    @testset "non-exact same-side coalescence is deferred" begin
        upper_a = causal_expression_denominator(1, -Eₐ, -1)
        upper_b = causal_expression_denominator(1, -Eₐ, -2)
        expression = one_term_expression(1 // 1, [upper_a, upper_b])
        result = @inferred KC.integrate_causal_frequency_expression(expression, 1)
        @test KC.causal_frequency_integration_kind(result) ===
            KC.CausalFrequencyRepeatedPole
    end

    @testset "opposite-side coincident poles are deferred as a pinch" begin
        upper = causal_expression_denominator(1, -Eₐ, -1)
        lower = causal_expression_denominator(1, -Eₐ, 1)
        expression = one_term_expression(1 // 1, [upper, lower])
        result = @inferred KC.integrate_causal_frequency_expression(expression, 1)
